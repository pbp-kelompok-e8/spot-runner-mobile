...
class EventEntry {
  ...
  factory EventEntry.fromJson(Map<String, dynamic> json) {
    ...
    Map<String, dynamic> userEoJson = json["user_eo"] is Map
        ? Map<String, dynamic>.from(json["user_eo"])
        : {};
    ...
    return EventEntry(
      ...
      eventCategories: _parseCategories(json["event_categories"]),
    );
  }

  // Helper untuk parse event_categories dari berbagai format
  static List<String> _parseCategories(dynamic categories) {
    if (categories == null) return <String>[];
    ...
    return categories
        .map<String>((item) {
          ...
          if (item is Map) {
            // Handle format: {"name": "Marathon"} atau {"category_name": "Marathon"}
            return (item['name'] ??
                        item['category_name'] ??
                        item['category'] ??
                        item.values.first)
...
class UserEo {
  ...
  UserEo({required this.id, required this.username});
  ...
  factory UserEo.fromJson(Map<String, dynamic> json) {
    ...
    return UserEo(id: parsedId, username: json["username"]?.toString() ?? '');
  }
